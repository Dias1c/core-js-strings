...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((prev, item) => item.charCodeAt(0) + prev, 0);
}
...
function isPalindrome(str) {
  ...
  const p1 = splitted.slice(0, splitted.length / 2).join('');
  const p2 = splitted
    .slice(0 - splitted.length / 2, splitted.length)
    .reverse()
    .join('');
...
function encodeToRot13(str) {
  return str
    ...
    .map((c) => {
      ...
      let starterCodePoint = 'A'.charCodeAt(0);
      if (isLower) {
        starterCodePoint = 'a'.charCodeAt(0);
      }

      let charCode = c.charCodeAt(0) - starterCodePoint;
      charCode += 13;
      if (charCode >= 26) charCode %= 26;
      return String.fromCharCode(charCode + starterCodePoint);
    })
    .join('');
}
